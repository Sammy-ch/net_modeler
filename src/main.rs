...
impl Network {
    ...
    fn add_link(&mut self, link: Link) -> Result<(), NetworkError> {
...
fn main() -> Result<(), Box<dyn Error>> {
    println!("Starting Network Modeling tool");
    ...
    println!(
        "Network successfully loaded with {} nodes and {} links.",
        network.graph.node_count(),
        network.graph.edge_count()
    );

    // Example: Print the graph in DOT format (can be visualized with Graphviz)
    println!("\nDOT representation of the network:");
    println!(
        "{:?}",
        Dot::with_config(&network.graph, &[Config::EdgeNoLabel])
    );

    Ok(())
}

fn load_network_links() -> Result<Vec<Link>, NetworkError> {
...
mod test {
    ...
    fn network_load_successful() {
        ...
        let network_links = load_network_links().expect("Failed to load network links");
        assert!(!network_links.is_empty());
        assert_eq!(network_links.len(), 3);
